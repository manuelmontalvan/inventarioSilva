...
import {
  ...
  updateCustomer,
  deleteCustomer,
} from "@/lib/api/sales/customers"; // Asegúrate de tener estas funciones en tu API
...
function CustomerDrawer({
  ...
  // Cuando cambie initialData, carga valores para editar
  useEffect(() => {
    if (initialData) {
  ...
  // Limpia el formulario al cerrar el drawer
  useEffect(() => {
    if (!isOpen) {
      reset();
    }
          ...
          <label key={field.name} className="flex flex-col">
            ...
            {errors[field.name as keyof CustomerForm] && (
              <span className="text-red-500 text-sm">
                {
                  errors[field.name as keyof CustomerForm]?.message as
                    | string
                    | undefined
                }
              </span>
